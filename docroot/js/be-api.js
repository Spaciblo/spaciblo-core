...
/*
Back End (be) code that is not specific to Spaciblo:
- auth functions like login/logout
- Potassium DataModels and DataCollections generated by the self-describing backend API

On page load, the schema description will be fetched and be.api.* will be populated.
When it is complete, a be.events.SchemaPopulatedEvent event will be dispatched on document.
Listen for that like so:

	document.addEventListener(be.events.SchemaPopulated, () => {
		init the page here...
	})

Once the be.events.SchemaPopulatedEvent has been dispatched, you can also rely on
the existence (but not population) of be.api.CurrentUser, which is a be.api.User
instance representing the person viewing the site.

The user's information is fetched on each page load and (if authenticated) is stored locally.
On page load, the locally stored data is used while the initial fetch is performed. So, if the
page relies on an authenticated user, listen on be.api.CurrentUser for fetch events and
